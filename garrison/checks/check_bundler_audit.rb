        ...
        if options[:github_private_key_base64]
          ...
          installations.each do |installation|
            ...
            repositories.each do |repo|
              ...
              Logging.info "#{repo.full_name} - Looking for Gemfile.lock"
              Dir.mktmpdir(repo.full_name.tr('/', '-')) do |dir|
                begin
                  gemfile = org.contents(repo.full_name, path: 'Gemfile.lock')
                  File.open(File.join(dir, 'Gemfile.lock'), 'w') { |f| f.write(Base64.decode64(gemfile[:content])) }
                  Logging.info "#{repo.full_name} - Scanning"
                  scan_lock_file(repo, dir)
        ...
        scanner.scan.each do |result|
          ...
          when Bundler::Audit::Scanner::UnpatchedGem
            alert(
              ...
              finding_id: "#{repo.full_name}-#{result.gem.name}-#{result.advisory.id}",
              urls: [
                {
